export const recipes = [
  {
    id: 1,
    img: 'https://images.immediate.co.uk/production/volatile/sites/30/2020/08/gnocchi-cacio-e-pepe-a1392fd.jpg?quality=90&webp=true&resize=440,400',
    title: 'Gnocchi Caccio e Peppe',
    rate: 4,
  },
  {
    id: 2,
    img: 'https://images.immediate.co.uk/production/volatile/sites/30/2020/08/veggie-fajitas-d49e108.jpg?quality=90&webp=true&resize=440,400',
    title: 'Fajitas Vegetarianas',
    rate: 4.5,
  },
  {
    id: 3,
    img: 'https://images.immediate.co.uk/production/volatile/sites/30/2020/08/Kale-Pesto-df3faaf.jpg?quality=90&webp=true&resize=375,341',
    title: 'Pasta al Pesto Vegana',
    rate: 3,
  },
  {
    id: 4,
    img: 'https://images.immediate.co.uk/production/volatile/sites/30/2020/08/vegan-ramen-c656140.jpg?quality=90&webp=true&resize=440,400',
    title: 'Kioto Ramen Vegano',
    rate: 4,
  },
  {
    id: 5,
    img: 'https://images.immediate.co.uk/production/volatile/sites/30/2020/08/recipe-image-legacy-id-326597_11-b7385b9.jpg?quality=90&webp=true&resize=440,400',
    title: 'Hamburguesas de Falafel',
    rate: 1,
  },
  {
    id: 6,
    img: 'https://images.immediate.co.uk/production/volatile/sites/30/2020/08/recipe-image-legacy-id-399680_11-3582ff5.jpg?quality=90&webp=true&resize=440,400',
    title: 'Fideos picantes de brocoli',
    rate: 2,
  }
];

export const products = [
  {
    id: 1,
    img: "https://i0.wp.com/www.dondecomersano.es/wp-content/uploads/2019/04/Plan-de-marketing-restaurante-vegetariano-para-locales-mixtos.jpg?resize=750%2C480",
    title: "Restaurante Vegetariano Primavera",
    favorito: false,
    details: `<li>Dirección: 3era calle 17-88 zona 8 de Mixco, ciudad San Cristobal Guatemala, City 01057</li>
                <li>Horario: Cerrado ⋅ Abre 9AM Martes</li>
                <li>Medidas de seguridad: Mascaría obligatoria · Chequeo de temperatura · Empleados con mascarilla obligatoria y careta.</li>
                <li>Menu: primavera.com.gt</li>
                <li>Teléfono: 2485 - 4210</li>`,
    lat: 14.5938643,
    lng: -90.5822611,
    rate: 4,
  },
  {
    id: 2,
    img: "https://stylelovely.com/wp-content/uploads/Los-mejores-restaurantes-vegetarianos.jpg",
    title: "Le Café Sur Plaza",
    favorito: false,
    details: `<li>Dirección: Calzada Aguilar Batres 31-36 zona 11 Centro Comercial Sur Plaza, Cdad. de Guatemala 01011</li>
                <li>Horario: Cerrado ⋅ Abre 9AM Martes</li>
                <li>Menu: primavera.com.gt</li>
                <li>Teléfono: 2485 - 4210</li>`,
    lat: 14.5938643,
    lng: -90.5822611,
    rate: 4.6,
  },
  {
    id: 3,
    img: "https://www.sillasmesas.es/blog/wp-content/uploads/2020/02/comida-restaurante-salad-Planet.jpg",
    title: "Moonrise Comida Vegana",
    favorito: false,
    details: `<li>Dirección: Via 6, Guatemala Ruta 3, Edificio Rue 3 Local 1 Guatemala, 01004</li>
                <li>Horario: Cerrado ⋅ Abre 9AM Martes</li>
                <li>Menu: primavera.com.gt</li>
                <li>Teléfono: 2485 - 4210</li>`,
    lat: 14.5938643,
    lng: -90.5822611,
    rate: 4.8,
  },
  {
    id: 4,
    img: "https://forum.com.gt/files/user10/RESTAURANTES%20VEGETARIANOS/Arbol%20de%20la%20vida.jpg",
    title: "Sat Nam Cafe vegetariano&vegano",
    favorito: false,
    details: `<li>Dirección: 4TA AVENIDA 12-29 ZONA 14 INTERIOR DEL MERCADITO 14 Guatemala, 01014</li>
                <li>Horario: Cerrado ⋅ Abre 9AM Martes</li>
                <li>Menu: lavka.com.gt</li>
                <li>Teléfono: 2485 - 4210</li>`,
    lat: 14.5938643,
    lng: -90.5822611,
    rate: 5,
  },
  {
    id: 5,
    img: "https://enlacocina.b-cdn.net/wp-content/uploads/2020/02/Plan-de-marketing-de-un-restaurante-vegetariano-1020x640.jpg",
    title: "Saberico Delicatessen",
    favorito: false,
    details: `<li>Dirección: 13 Calle 2-33, Cdad. de Guatemala 01010</li>
                <li>Horario: Cerrado ⋅ Abre 9AM Martes</li>
                <li>Teléfono: 2485 - 4210</li>`,
    lat: 14.5938643,
    lng: -90.5822611,
    rate: 6,
  },
  {
    id: 6,
    img: "https://images.adsttc.com/media/images/5ab9/3634/f197/cc6e/5400/00b7/newsletter/0376.jpg?1522087455",
    title: "El Mercadito de Lola",
    favorito: false,
    details: `<li>Dirección: 11 Calle 5-51, Cdad. de Guatemala</li>
                <li>Horario: Cerrado ⋅ Abre 9AM Martes</li>
                <li>Teléfono: 2485 - 4210</li>`,
    lat: 14.5938643,
    lng: -90.5822611,
    rate: 1,
  },
];
